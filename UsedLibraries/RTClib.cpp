...
uint8_t RTC_DS1307::isrunning(void) {
  ...
  */
  ...
  write_address |= DS1307_ADDRESS << 1;
  //Serial.print("write_address is ");
  //Serial.println(write_address,HEX);
  customWire._I2C_WRITE(write_address);
  //customWire._I2C_WRITE(DS1307_ADDRESS);
  customWire._I2C_WRITE((byte)0);
  ...
  //customWire._I2C_WRITE(DS1307_ADDRESS);
  uint8_t ss = 0;
  ss = customWire._I2C_READ(0);  
  customWire.endTransmission();
  //Serial.print("ss is ");
  //Serial.println(ss>>7,HEX);
  return !(ss>>7);
}
...
void RTC_DS1307::adjust(const DateTime& dt) {
  ...
  */
  ...
  customWire._I2C_WRITE(write_address);
  //customWire._I2C_WRITE(DS1307_ADDRESS);
  customWire._I2C_WRITE((byte)0); // start at location 0

  customWire._I2C_WRITE(bin2bcd(dt.second()));
  customWire._I2C_WRITE(bin2bcd(dt.minute()));
  customWire._I2C_WRITE(bin2bcd(dt.hour()));
  customWire._I2C_WRITE(bin2bcd(0));
  customWire._I2C_WRITE(bin2bcd(dt.day()));
  customWire._I2C_WRITE(bin2bcd(dt.month()));
  customWire._I2C_WRITE(bin2bcd(dt.year() - 2000));  
  customWire.endTransmission();
}
...
*/
  ...
  customWire.beginTransmission();
  byte read_address = 1;
  read_address |= DS1307_ADDRESS << 1;
  customWire._I2C_WRITE(read_address);
  //customWire._I2C_WRITE(DS1307_ADDRESS + 1);
  //------------------------------------------------
  
  uint8_t ss = bcd2bin(customWire._I2C_READ(1)& 0x7F);  
  
  uint8_t mm = bcd2bin(customWire._I2C_READ(1));
  uint8_t hh = bcd2bin(customWire._I2C_READ(1));
  customWire._I2C_READ(1);
  uint8_t d = bcd2bin(customWire._I2C_READ(1));
  uint8_t m = bcd2bin(customWire._I2C_READ(1));
  uint16_t y = bcd2bin(customWire._I2C_READ(0)) + 2000;
  
  //return DateTime (y, m, d, hh, mm, ss);
  customWire.endTransmission();
  //return DateTime (0, 0 ,0, 0, 0, ss);
  return DateTime (y, m, d, hh, mm, ss);
}
